...
readonly CORE_DIR="$( cd "$( dirname "${BASH_SOURCE[0]}" )" >/dev/null 2>&1 && cd .. && pwd )"
readonly PLATFORM_REPO_DIR="$( cd "$( dirname "${BASH_SOURCE[0]}" )" >/dev/null 2>&1 && cd ../.. && pwd )"
readonly PLATFORM_SCRIPT_DIR="${PLATFORM_REPO_DIR}/scripts"

# Source the deploy-vars.sh file
if [ -f ${PLATFORM_SCRIPT_DIR}/deploy-vars.sh ]; then
    source ${PLATFORM_SCRIPT_DIR}/deploy-vars.sh
fi
...
set_server_user() {
    ...
    case "${buildType}" in
        "production")
            ...
            check_variable ${serverUser} "PRODUCTION_SERVER_USER environment variable not set."
            check_variable ${documentRoot} "PRODUCTION_DOCUMENT_ROOT environment variable not set."
            check_variable ${domainRoot} "PRODUCTION_DOMAIN_ROOT environment variable not set."
            check_variable ${sshIdentityFile} "PRODUCTION_SSH_IDENTITY_FILE environment variable not set."
            check_variable ${sshHost} "PRODUCTION_SSH_HOST environment variable not set."
            check_variable ${sshUser} "PRODUCTION_SSH_USER environment variable not set."
            ;;
        "staging")
            ...
            # ensure all variables are set
            check_variable ${serverUser} "STAGING_SERVER_USER environment variable not set."
            check_variable ${documentRoot} "STAGING_DOCUMENT_ROOT environment variable not set."
            check_variable ${domainRoot} "STAGING_DOMAIN_ROOT environment variable not set."
            check_variable ${sshIdentityFile} "STAGING_SSH_IDENTITY_FILE environment variable not set."
            check_variable ${sshHost} "STAGING_SSH_HOST environment variable not set."
            check_variable ${sshUser} "STAGING_SSH_USER environment variable not set."
            ;;
        *)
...
run_main() {
    ...
    set_server_user
    check_variable ${serverUser} "SERVER_USER environment variable not set."
    ...
    if [[ $runFrontend == true || $runBackend == true ]]; then
        ...
        cd ${PLATFORM_REPO_DIR} && npm run core build:${buildType}
    fi
