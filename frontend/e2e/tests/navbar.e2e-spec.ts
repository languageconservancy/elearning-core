...
describe('Navbar', () => {
	...
	describe('learn button', () => {
		...
		it(`should have background of ${ColorThemeRgba.PRIMARY_COLOR_DARK}
			...
			expect(await learnBtn.getCssValue('background-color'))
				.toEqual(ColorThemeRgba.PRIMARY_COLOR_DARK);
			expect(await learnBtn.getCssValue('color'))
		...
		it(`hovered should have background of ${ColorThemeRgba.PRIMARY_COLOR_DARK}
			...
			await browser.wait(EtoELib.waitForCssValue(
				learnBtn, 'background-color', ColorThemeRgba.PRIMARY_COLOR_DARK), EtoEConsts.CSS_WAIT_TIMEOUT_MS);
			await browser.wait(EtoELib.waitForCssValue(
	...
	describe('review button', () => {
		...
		it(`should have background of rgba(0, 0, 0, 0)
			and color of ${ColorThemeRgba.TEXT_PRIMARY_CONTRAST}`, async() => {
			expect(await reviewBtn.getCssValue('background-color'))
				.toEqual('rgba(0, 0, 0, 0)');
			expect(await reviewBtn.getCssValue('color'))
				.toEqual(ColorThemeRgba.TEXT_PRIMARY_CONTRAST);
		});
		...
		it(`hovered should have background of ${ColorThemeRgba.PRIMARY_COLOR_DARK}
			...
			await browser.actions()
				...
				.perform();
			await browser.wait(EtoELib.waitForCssValue(
				reviewBtn, 'background-color', ColorThemeRgba.PRIMARY_COLOR_DARK), EtoEConsts.CSS_WAIT_TIMEOUT_MS);
			await browser.wait(EtoELib.waitForCssValue(
	...
	describe('user menu', () => {
		...
		it(`should have transparent background and color of ${ColorThemeRgba.TEXT_PRIMARY_CONTRAST}`, async() => {
			expect(await usernameText.getCssValue('background-color'))
				.toEqual('rgba(0, 0, 0, 0)');
			expect(await usernameText.getCssValue('color'))
				.toEqual(ColorThemeRgba.TEXT_PRIMARY_CONTRAST);
		});

		it(`should drop down when hovered`, async() => {
			await browser.actions()
				.mouseMove(usernameText)
				.perform();
			await browser.wait(EtoELib.waitForCssValue(
				profileDropdown, 'display', 'block'), EtoEConsts.CSS_WAIT_TIMEOUT_MS);
		});
		...
		it(`hovered dropdown menu items should have background of ${ColorThemeRgba.PRIMARY_COLOR}
			and color of ${ColorThemeRgba.TEXT_UI_PANEL_DARK_CONTRAST}`, async() => {
			await browser.actions()
				.mouseMove(settingsDropdownItem)
				.perform();
			await browser.wait(EtoELib.waitForCssValue(
				settingsDropdownItem, 'background-color', ColorThemeRgba.PRIMARY_COLOR), EtoEConsts.CSS_WAIT_TIMEOUT_MS);
			await browser.wait(EtoELib.waitForCssValue(
					settingsDropdownItem, 'color', ColorThemeRgba.TEXT_PRIMARY_CONTRAST), EtoEConsts.CSS_WAIT_TIMEOUT_MS);
		});
	});
});
